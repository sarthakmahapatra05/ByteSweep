...
const LargeFileAnalyzer = () => {
  ...
  const scanLargeFiles = async () => {
    setIsScanning(true);
    try {
      const res = await api.get(`/files/large/${sizeThreshold * 1024 * 1024}`);
      setLargeFiles(res.data.files || []);
    } catch (err) {
      setLargeFiles([]);
    }
    setIsScanning(false);
      ...
      >
        {modalFile && (
          <div>
            ...
            <p className="text-gray-700 mb-1">Size: {modalFile.size}</p>
            <p className="text-gray-700 mb-1">Type: {modalFile.type}</p>
            <p className="text-gray-700 mb-1">Last Modified: {modalFile.lastModified}</p>
        ...
        <div className="bg-white rounded-lg p-6 shadow-md border border-gray-100">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">{largeFiles.length}</p>
            </div>
          ...
          {selectedFiles.size > 0 && (
            <div className="flex items-center gap-4">
              <span className="text-sm text-gray-600">
                {selectedFiles.size} files selected ({selectedSize})
              </span>
      ...
      <div className="bg-white rounded-lg shadow-md border border-gray-100 overflow-hidden">
        <div className="p-4 bg-gray-50 border-b border-gray-200">
          <h3 className="text-lg font-semibold text-gray-900">
            Large Files ({filteredAndSortedFiles.length})
          </h3>
        </div>
        {isScanning ? (
