...
const TempFileManager = () => {
  ...
  const scanTempFiles = async () => {
    setIsScanning(true);
    try {
      const res = await api.get('/files/category/temp');
      setTempFiles(res.data.files || []);
    } catch (err) {
      setTempFiles([]);
    }
    setIsScanning(false);
  ...
  const downloadReport = () => {
    ...
    const csv = [
      ...
      ...deletedFilesReport.map(f => `${f.name},${f.path},${f.size},${f.type},${f.lastModified.toLocaleDateString()}`)
    ].join('\n');
          ...
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-green-700 text-sm">
                Removed {cleanupResult.filesRemoved} files and freed {f.formatBytes(cleanupResult.spaceFreed)}
              </p>
            </div>
          </div>
          ...
          {selectedFiles.size > 0 && (
            <div className="flex items-center gap-4">
              <span className="text-sm text-gray-600">
                {selectedFiles.size} files selected ({f.formatBytes(totalSelectedSize)})
              </span>
              <button
      ...
      <div className="bg-white rounded-lg shadow-md border border-gray-100 overflow-hidden">
        <div className="p-4 bg-gray-50 border-b border-gray-200">
          <h3 className="text-lg font-semibold text-gray-900">
            Temporary Files ({filteredFiles.length})
          </h3>
        </div>
        {isScanning ? (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs text-gray-500 ml-2">{f.formatBytes(file.size)}</span>
                  </div>
